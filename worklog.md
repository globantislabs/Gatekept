...
---
Task ID: 20
Agent: Super Z (order/product deletion + NJMSPN order IDs + scan dedupe + build-safe uploads + IST dates)
Task: (a) products must be deletable even when linked to orders; (b) orders need a delete option in admin; (c) order IDs auto-generated as NJMSPN001-style everywhere (WhatsApp/mail/all); (d) campaign QR records one scan ~7 times; (e) npm run build must NOT delete uploaded videos/images; (f) dates should show the current IST date

Work Log:
- Product delete unblocked: OrderItem.product and Subscription.product were RESTRICT FKs — schema (mysql + sqlite variants) now has nullable product_id with onDelete: SetNull; both models keep full snapshots (product_name, product_type, unit_price, pack info) so order/subscription history stays complete. DELETE /api/products/[id] now detaches order items, subscriptions and campaigns (best-effort updateMany) and explicitly removes videos/quizzes/learning-progress, then deletes the product; graceful fallback message if the prod DB FKs have not been pushed yet
- Order deletion added: new DELETE /api/orders/[id] (admin-only via checkAdmin; children cascade — items, tracking, subscription, invoice) + orderService.delete in data-service + Delete buttons in the admin Orders tab (desktop table row and mobile card), each with a confirmation warning that items/invoice/subscription are removed too
- Order numbers: replaced the NJ+base36-timestamp+random generator with a sequential NJMSPN001, NJMSPN002, ... generator (max numeric suffix + padStart(3)), retried up to 5 times on a unique-constraint race; order_number is already used by WhatsApp/email/SMS templates, invoices, profile and checkout, so the new format propagates everywhere automatically; admin Orders table and detail dialog now display order_number instead of the raw cuid id
- Scan dedupe: POST /api/scans now returns the existing record when an identical scan (same campaign_id + device) was already recorded within the last 60 seconds — QR scanner apps and in-app browsers prefetch/reload the landing URL several times per physical scan, which is how one scan appeared as ~7 rows; client-side session guard kept as first line of defense
- Uploads survive builds: root cause — the Next standalone server runs with process.chdir(.next/standalone), so the upload route's process.cwd()-based data/uploads resolved INSIDE .next, which `next build` regenerates from scratch, wiping every uploaded video/image on each build (and build-plesk rm -rf's .next explicitly). server.js now sets UPLOAD_ROOT=<project>/data/uploads before loading the standalone server and pre-creates the folders; upload + serve routes resolve the root via UPLOAD_ROOT (with a standalone-cwd safety net and legacy/public read fallbacks); dev (next dev) behavior unchanged
- Invoice dates pinned to Asia/Kolkata: a UTC-server invoice showed yesterday's date for orders placed 00:00-05:30 IST; issued_at and order.created_at now render in IST explicitly
- Live-verified against a dev server + fresh SQLite DB (12/12 checks): NJMSPN001/NJMSPN002 sequential numbers; product with linked order deletes and the order item keeps its product_name snapshot with product_id detached; admin order delete works + 404 on missing + unauthorized for non-admins; first scan POST 201 then identical posts deduped, exactly 1 scan row recorded
- Verified: tsc error profile byte-identical before/after via git stash diff (0 new); next build PASSES (SQLite swap, MySQL schema restored byte-identical, Prisma client regenerated); temp test DB removed; schema files verified MySQL after verification
